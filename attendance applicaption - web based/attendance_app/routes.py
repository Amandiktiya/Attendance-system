...
def student_sort_sql(alias="users"):
    return f"""
CASE
    WHEN COALESCE(NULLIF({alias}.class_roll_number, ''), '') GLOB '[0-9]*'
    THEN CAST({alias}.class_roll_number AS INTEGER)
    ELSE 999999999
END,
{alias}.class_roll_number,
CASE
    WHEN COALESCE(NULLIF({alias}.roll_number, ''), '') GLOB '[0-9]*'
    THEN CAST({alias}.roll_number AS INTEGER)
    ELSE 999999999
END,
{alias}.roll_number,
{alias}.full_name
...
def grouped_students(rows):
    groups = OrderedDict()
    for row in rows:
        normalized_year = normalize_numeric_label(row["year"], "Year")
        normalized_semester = normalize_numeric_label(row["semester"], "Semester")
        branch = (row["branch"] or "-").strip() or "-"
        key = (branch, normalized_year["sort"], normalized_semester["sort"])
        if key not in groups:
            groups[key] = {
                "branch": branch,
                "year": normalized_year["label"],
                "semester": normalized_semester["label"],
                "students": [],
            }
...
def grouped_faculties(rows):
    groups = OrderedDict()
    for row in rows:
        branch = (row["branch"] or "Unassigned").strip() or "Unassigned"
        if branch not in groups:
            groups[branch] = {"branch": branch, "faculties": []}
        groups[branch]["faculties"].append(row)
    return list(groups.values())


def normalize_numeric_label(value, prefix):
    raw_value = (value or "").strip()
    match = re.search(r"\d+", raw_value)
    if match:
        number = int(match.group())
        return {"label": f"{prefix} {number}", "sort": number}
    if raw_value:
        return {"label": f"{prefix} {raw_value}", "sort": 999999}
    return {"label": f"{prefix} -", "sort": 999999}
...
def dashboard():
    ...
    params = []
    if user["role"] == "faculty":
        student_query += " AND " + faculty_data_clause("student_details")
        params.extend([user["institution_type"], user["institution_name"], user["branch"]])
    if selected_branch:
        student_query += " AND branch = ?"
        params.append(selected_branch)
    student_query, params = add_semester_group_filter(student_query, params, "student_details.semester", selected_year_group)
    student_query += " ORDER BY " + student_sort_sql("student_details")
    ...
    faculty_params = []
    if user["role"] == "faculty":
        faculty_query += " AND " + faculty_scope_clause("faculty_details")
...
def attendance():
    ...
    query += " ORDER BY " + student_sort_sql("users")
