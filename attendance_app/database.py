...
def get_db():
    if "db" not in g:
        configured_path = Path(current_app.config["DATABASE"])
        db_path = configured_path if configured_path.is_absolute() else Path(current_app.root_path).parent / configured_path
